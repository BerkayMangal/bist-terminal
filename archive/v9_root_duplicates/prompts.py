...
def agent_prompt(context: str, query: str) -> str:
    """Build the Q agent AI prompt — V13: Academic-aware."""
    return (
        "Sen Q'sun — BistBull asistanı. Kurumsal, kısa, Türkçe. 3-5 cümle MAX.\n\n"
        "ÖNEMLİ KURALLAR:\n"
        "1. Türkiye bağlamı: yüksek faiz, enflasyon, kur riski DAİMA dikkate al.\n"
        "2. turkey_realities varsa filtrelerin puanı neden kırdığını açıkla.\n"
        "3. academic varsa Damodaran/Greenwald çerçevesinde yorumla:\n"
        "   • ROE'yi sermaye maliyetiyle (Ke) karşılaştır, excess return söyle.\n"
        "   • Büyümeyi reel ver (nominal - enflasyon).\n"
        "   • FCF negatifken büyümeyi övme — 'büyüme tuzağı' de.\n"
        "   • Olgun şirkette düşük temettüyü eleştir.\n"
        "4. Rakam kullan, genel laflar yapma.\n"
        "5. YASAK: kesinlikle, garanti, kaçırma, patlayacak, uçacak, hemen al.\n\n"
        f"{context}"
        f"Soru: {query}\n\nQ:"
    )
...
def build_rich_context(r: dict, tech: Optional[dict] = None) -> str:
    ...
    # V12: Turkey Realities context
    tr = r.get("turkey_realities")
    if tr and tr.get("filters"):
        lines.append("\n--- TÜRKİYE GERÇEKLERİ ---")
        lines.append(f"Not: {tr.get('composite_grade', '?')} | Çarpan: {tr.get('composite_multiplier', 1.0):.2f}x")
        for _fk, _fv in tr.get("filters", {}).items():
            if isinstance(_fv, dict):
                lines.append(f"  {_fv.get('name', _fk)}: {_fv.get('grade', '?')} — {_fv.get('explanation', '')}")
        if tr.get("adjusted_fa"): lines.append(f"Turkey-Adj FA: {tr['adjusted_fa']}")
        lines.append(f"Özet: {tr.get('summary', '')}")

    # V13: Academic context
    ac = r.get("academic")
    if ac and ac.get("filters"):
        lines.append("\n--- AKADEMİK (Damodaran + Greenwald) ---")
        lines.append(f"Not: {ac.get('composite_grade', '?')} | Etki: {ac.get('composite_penalty', 0):+d}p")
        for _ak, _av in ac.get("filters", {}).items():
            if isinstance(_av, dict):
                lines.append(f"  {_av.get('name', _ak)}: {_av.get('grade', '?')} [{_av.get('label', '')}] — {_av.get('explanation', '')}")
                comps = _av.get("components", {})
                if "excess_return" in comps:
                    lines.append(f"    Excess Return: %{comps['excess_return']*100:+.0f}")
                if comps.get("is_trap"):
                    lines.append(f"    ⚠️ BÜYÜME TUZAĞI")
                if comps.get("real_growth_pct") is not None:
                    lines.append(f"    Reel Büyüme: %{comps['real_growth_pct']:+.0f}")
        if ac.get("adjusted_fa"): lines.append(f"Final FA (TR+Akademik): {ac['adjusted_fa']}")
...
def trader_summary_prompt(r: dict, tech: Optional[dict] = None) -> str:
    ...
    V13: Turkey Realities + Academic Layer aware."""
    ...
    # V12: Turkey instruction
    tr = r.get("turkey_realities")
    turkey_inst = ""
    if tr and tr.get("composite_grade"):
        cg = tr["composite_grade"]
        weak = [f.get("name", k) for k, f in tr.get("filters", {}).items() if isinstance(f, dict) and f.get("grade") in ("D", "F")]
        turkey_inst = f"\n🇹🇷 TÜRKİYE: Not {cg}. "
        if weak: turkey_inst += f"ZAYIF: {', '.join(weak)} — TÜRKİYE satırında açıkla. "

    # V13: Academic instruction
    ac = r.get("academic")
    acad_inst = ""
    if ac and ac.get("composite_grade"):
        acg = ac["composite_grade"]
        acp = ac.get("composite_penalty", 0)
        acad_inst = f"\n🎓 AKADEMİK: Not {acg}, etki {acp:+d}p. "
        vc = ac.get("filters", {}).get("value_creation", {})
        if isinstance(vc, dict) and vc.get("grade") in ("D", "F"):
            comps = vc.get("components", {})
            acad_inst += f"DEĞER YIKIMI: ROE %{comps.get('roe',0)*100:.0f} < Ke %{comps.get('ke',0.58)*100:.0f}. Belirt. "
        gt = ac.get("filters", {}).get("growth_trap", {})
        if isinstance(gt, dict) and gt.get("is_trap"):
            acad_inst += "BÜYÜME TUZAĞI tespit edildi — RİSK'te belirt. "

    return (
        "Sen kurumsal BIST analisti ve portföy yöneticisisin. 20 yıllık Türkiye tecrüben var.\n"
        "Damodaran ve Greenwald perspektifinden düşünürsün.\n"
        "Aşağıdaki veriye dayanarak yatırım tezi yaz. Türkçe.\n\n"
        "ASLA sallama, SADECE verideki rakamlara dayan.\n"
        "YASAK: kesinlikle, garanti, kaçırma, patlayacak, uçacak, hemen al.\n\n"
        "V13 KURALLAR:\n"
        "- ROE'yi sermaye maliyeti (Ke) ile karşılaştır. Excess return pozitifse değer yaratımı, negatifse yıkım.\n"
        "- Büyümeyi HER ZAMAN reel ver (nominal - enflasyon).\n"
        "- FCF negatifken büyümeyi övme — büyüme tuzağı uyarısı ver.\n"
        "- Olgun şirkette düşük temettüyü eleştir.\n"
        f"{'⚠️ HYPE/SPEKÜLATİF — temel zayıf ama fiyat uçuyor.' if is_hype else ''}\n"
        f"{quality_note}"
        f"{turkey_inst}"
        f"{acad_inst}\n"
        f"{ctx}\n\n"
        "Şu formatta yaz (her satır ayrı, başka HİÇBİR ŞEY yazma):\n"
        f"GİRİŞ: {entry} — bu ne anlama geliyor? 1 cümle.\n"
        "TEZ: 2 cümle — excess return, reel büyüme, nakit yaratımı perspektifinden. Rakam kullan.\n"
        "RİSK: 1 cümle — değer yıkımı, büyüme tuzağı veya sermaye israfı varsa. Rakam kullan.\n"
        "ZAMANLAMA: 1 cümle — yaşam döngüsü fazını dikkate alarak.\n"
        "TÜRKİYE: 1 cümle — Türkiye + akademik filtre birleşik sonucu.\n"
    )
