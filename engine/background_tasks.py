...
def _fetch_heatmap_data() -> list[dict]:
    ...
    try:
        ...
        for t in UNIVERSE:
            ...
            try:
                ticker_df = (
                    df if len(UNIVERSE) == 1
                    else (df[sym] if sym in df.columns.get_level_values(0) else None)
                )
                ...
                results.append({
                    ...
                    "market_cap": si["metrics"].get("market_cap") if si else None,
                    "sector":     (si.get("sector", "Diger") if si else "Diger") or "Diger",
                    "score":      si["overall"] if si else None,
                })
            except Exception:
                continue
        log.debug(f"Heatmap yfinance: {len(results)} hisse")
    except Exception as e:
        log.warning(f"Heatmap yfinance hatasi: {e}")
                    ...
                    if raw_df is not None and not raw_df.empty:
                        for t in tickers:
                            ...
                            try:
                                ...
                                log.debug(f"PaperTrade parse [{t}]: {pe}")
                    log.debug(
                        f"PaperTrade yfinance: {len(price_map)}/{len(tickers)} fiyat alindi"
                    )
                except Exception as e:
                    log.warning(f"PaperTrade yfinance batch hatasi: {e}")
