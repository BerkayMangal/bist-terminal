...
h+='<table class="dtb"><thead><tr><th>#</th><th>Hisse</th><th>Fiyat</th><th title="Veri tazeliği — borsapy fetch yaşı + KAP son finansal rapor">📅 Veri</th><th title="Radar kalite notu">Not</th><th style="color:var(--grn)">Değer</th><th>Değ</th><th>Kal</th><th>Bil</th><th>ROE</th><th>F/K</th><th>Ciro/PD</th></tr></thead><tbody>';
sorted.forEach((it,i)=>{const dc=it.decision;const dCol=vColor(dc);const dcLabel=vLabel(dc);const cpL=it.ciro_pd_label;const cpBadge=cpL?`<span style="font-family:'JetBrains Mono',monospace;font-size:8px;font-weight:700;padding:1px 5px;border-radius:3px;background:${cpL.color}18;color:${cpL.color}">${cpL.label}</span>`:'<span style="color:var(--t4);font-size:9px">—</span>';const roeVal=it.roe;const roePct=roeVal!=null?`<span style="font-family:'JetBrains Mono',monospace;font-size:var(--fs-xs);color:${roeVal>=0.15?'var(--grn)':roeVal>=0.08?'var(--t1)':'var(--red)'};font-weight:${roeVal>=0.15?700:400}">${(roeVal*100).toFixed(0)}%</span>`:'<span style="color:var(--t4);font-size:9px">—</span>';const dqt=it.data_quality_tier||'full';const dqDot=dqt==='full'?'':'<span title="'+(dqt==='partial'?'Kısmi veri':'Sadece piyasa verisi')+'" style="font-size:7px;margin-left:2px">'+(dqt==='partial'?'🟡':'🔴')+'</span>';h+=`<tr${it.is_fatal?' style="opacity:0.5"':''}><td style="color:var(--t3)">${i+1}</td><td class="clk-t" onclick="loadTicker('${esc(it.ticker)}')">${esc(it.ticker)}${dqDot}${it.size_tier==='mikro'?'<span title="Mikro-cap — düşük likidite, dikkatli ol" style="font-size:7px;color:var(--ylw);margin-left:3px">mikro</span>':''}${it.is_fatal?'<span style="color:var(--red);font-size:8px"> ⛔</span>':''}</td><td style="font-family:'JetBrains Mono',monospace;font-size:var(--fs-xs);color:var(--t1)">${it.price!=null?fN(it.price,2):'—'}</td><td>${_radarFreshBadge(it.ticker)}</td><td><span style="font-family:'JetBrains Mono',monospace;font-size:var(--fs-xs);font-weight:700;color:${dCol};padding:2px 6px;background:${dCol}15;border-radius:3px">${dcLabel||'—'}</span></td><td><span style="font-weight:700;color:var(--grn)">${(it.deger||it.overall||0).toFixed(0)}</span></td><td>${sPill(it.scores?.value)}</td><td>${sPill(it.scores?.quality)}</td><td>${sPill(it.scores?.balance)}</td><td>${roePct}</td><td style="font-family:'JetBrains Mono',monospace;font-size:var(--fs-xs);color:var(--t1)">${it.pe?it.pe.toFixed(1):'—'}</td><td>${cpBadge}</td></tr>`;});
...
function _bwCard(item){
  ...
  const lp = item.metrics?.last_price;
  const priceStr = (lp!=null) ? `₺${fN(lp,2)}` : '';
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:8px">
      <div style="flex:1;min-width:0">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          ...
          ${priceStr?`<span style="font-family:'JetBrains Mono',monospace;font-size:14px;font-weight:600;color:var(--t1)">${priceStr}</span>`:''}
        </div>
